...
describe("Test FactorySwaptoStable", ()=> {
     ...
     it('Probando funsion contructorContract', async function () {
          ...
          this.factorySwapToStable = await FactorySwapToStable.attach(dataContractFactory.addressContract.mumbai);
          console.log('Address del contrato =>>',this.factorySwapToStable.address )

          console.log(1);
          
          //usando la funcion contructorContract
          const newContractReceipt = await this.factorySwapToStable.contructorContract("Tambos", owner.address, ethers.utils.parseEther("0.0001"))
          console.log(2);
          ...
          const contractsUsers1 = await this.factorySwapToStable.getContractsOfUsers(owner.address)
          console.log(`contrato del user1 ${owner.address} => `, contractsUsers1 );
          console.log(4);
          const contractsUsers2 = await this.factorySwapToStable.getContractsOfUsers(owner.address)
          ...
          const receipt2 = await this.factorySwapToStable.users(owner.address, 0)
